...
if ( ! class_exists( 'BWCC_Bulk_WooCommerce_Category_Creator' ) ) {
	...
	class BWCC_Bulk_WooCommerce_Category_Creator {
		...
		public function bwcc_enqueue_scripts() {
			...
			wp_enqueue_script(
				'bulk-wc-category-creator',
				plugins_url() . '/bulk-woocommerce-category-creator/js/bulk-wc-category-creator.js',
		...
		public static function bwcc_create_categories() {
			...
			if ( isset( $_POST['bwcc_options_textarea'] ) ) { // phpcs:ignore.
				...
				if ( '' !== $returned_str ) {
					...
					foreach ( $categories_array as $key => $value ) {
						...
						if ( $term == 0 || is_null( $term ) ) {
							$term_id = self::bwcc_create_category( $value, $parent_id, $description, $display_type );
							if ( $term_id ) {
